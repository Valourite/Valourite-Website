...
<html class="[color-scheme:dark] [--ink:#050b18] [--muted:#a6b5ce] [--blue:#50bdff] [--purple:#9275ff] [--line:#263754]
    ...
    <body class="m-0 [background:var(--ink)] text-[#f4f7ff] font-sans text-[16px] leading-[1.65] overflow-x-clip">
        ...
        desktop:[&_p:last-child]:text-[18px]">
                    <p class="eyebrow text-[11px] font-semibold tracking-[2.7px] text-[#a2a8ff] mb-[22px] mobile:mb-[18px] desktop:text-[13px]">OUR SERVICES</p>
                    <h2 class="leading-[1.15] text-[clamp(29px,_3vw,_46px)] font-semibold tracking-[-1.2px] desktop:text-[clamp(38px,_3.4vw,_54px)]" id="services-heading">What can we<br>build for you?</h2>
                    <p>Four ways to move your business forward.</p>
                </div>
                <div class="service-cards grid grid-cols-2 gap-x-[9%] gap-y-0 mobile:flex mobile:flex-col mobile:gap-[18px]">
                    @foreach ($services as $service)
                        <a class="[-webkit-tap-highlight-color:transparent] touch-manipulation no-underline
        focus-visible:[outline:2px_solid_#7dd3fc] focus-visible:outline-offset-[6px] service-card reveal card-{{ $loop->index }} relative overflow-hidden isolate flex items-start gap-[22px] p-[30px] rounded-[14px]
        ...
        reduce-motion:[&.is-waiting]:[transform:none]" href="#{{ $service['id'] }}">
                            @if ($service['image'])
                                <img src="{{ asset('images/' . $service['image'] . '-800.webp') }}" alt="" loading="lazy" width="800" height="530">
                            @endif
                            <x-icon :name="$service['icon']" />
                            <div><span class="card-number absolute right-[23px] top-5 text-[#7182a3] text-[10px] tracking-[2px]">0{{ $loop->iteration }}</span>
                                <h3 class="leading-[1.15] font-semibold">{{ $service['name'] }}</h3>
                                <ul>
                                    @foreach ($service['summary'] as $point)
                                        <li>{{ $point }}</li>
                                    @endforeach
                                </ul>
                            </div><span class="card-arrow absolute right-[19px] bottom-[18px] [border:1px_solid_#526788] rounded-full p-[6px]
        ...
        mobile:gap-[18px] desktop:[&_h2]:text-[43px]">
                        <div>
                            <p class="eyebrow text-[11px] font-semibold tracking-[2.7px] text-[#a2a8ff] mb-[22px] mobile:mb-[18px] desktop:text-[13px]">CLIENT FEEDBACK</p>
                            <h2 class="leading-[1.15] text-[clamp(29px,_3vw,_46px)] font-semibold tracking-[-1.2px] desktop:text-[clamp(38px,_3.4vw,_54px)]" id="reviews-heading">Good systems. Happy clients.</h2>
                        </div>
                        <div>
                            @if ($hasSamples)
                                <p class="sample-notice text-[11px] text-[#a5b4ca] desktop:text-[12px]">Sample testimonials · development preview only</p>
                            @endif
                        </div>
                    </div>
        ...
        [&.is-animated]:overflow-hidden reduce-motion:[&.is-animated]:overflow-x-auto">
                            <div class="review-track flex w-max gap-5 reduce-motion:[transform:none]!">
                                <div class="review-set flex gap-5 pr-0 reduce-motion:[&[data-clone]]:hidden">
        ...
        desktop:[&_small]:text-[12px]" tabindex="0">
                                            <p>{{ $testimonial['quote'] }}</p>
                                            <div class="stars text-[#8197ff] tracking-[3px] [margin:14px_0_7px]" aria-label="{{ $testimonial['rating'] }} out of 5 stars">{{ str_repeat('★', max(0, min(5, (int) $testimonial['rating']))) }}</div><span>{{ $testimonial['name'] }}</span>
                                            @if ($testimonial['sample'] ?? true)
        ...
        [clip-path:var(--section-edge)] mobile:min-h-145">
                            @if ($service['image'])
                                <picture class="detail-photo absolute [inset:0] z-[-2] [&_img]:w-full [&_img]:h-full [&_img]:object-cover [&_img]:object-center
        ...
        mobile:[&:after]:[background:linear-gradient(90deg,_#050e1bef,_#050e1ba6)]">
                                    <source media="(max-width: 700px)" srcset="{{ asset('images/' . $service['image'] . '-800.webp') }}"><img src="{{ asset('images/' . $service['image'] . '-1600.webp') }}" alt="{{ $service['image'] === 'automation' ? 'Laptop displaying business reporting dashboards' : 'Warehouse team managing stock with a tablet' }}" loading="lazy" width="1600" height="1060">
                                </picture>
                            @endif
        ...
        reduce-motion:[&.is-waiting]:opacity-[1] reduce-motion:[&.is-waiting]:[transform:none]">
                                    <p class="eyebrow text-[11px] font-semibold tracking-[2.7px] text-[#a2a8ff] mb-[22px] mobile:mb-[18px] desktop:text-[13px]">{{ strtoupper($service['name']) }}</p>
                                    <h2 class="leading-[1.15] text-[clamp(29px,_3vw,_46px)] font-semibold tracking-[-1.2px] desktop:text-[clamp(38px,_3.4vw,_54px)]" id="heading-{{ $service['id'] }}">{{ $service['heading'] }}</h2>
                                    <p class="service-description text-[#ccd5e4] text-[15px] mt-[22px] max-w-125 mobile:text-[15px] desktop:text-[20px]
        desktop:max-w-150">{{ $service['description'] }}</p>
                                    <ul class="service-points flex flex-col gap-[15px] [margin:30px_0] [&_li]:flex [&_li]:items-center [&_li]:gap-3
        [&_li]:text-[14px] [&_li]:text-[#d9e2f0] [&_.icon]:text-[#a393ff] [&_.icon]:w-[22px]
        mobile:[&_li]:text-[13px] mobile:[&_li]:items-start desktop:[&_li]:text-[18px]">
                                        @foreach ($service['points'] as $point)
                                            <li><x-icon name="check" /><span>{{ $point }}</span></li>
                                        @endforeach
                                    </ul>
                                    <x-contact-button :label="$service['cta']" />
                                </div>
                                @if ($service['id'] === 'websites')
                                    <div class="detail-art reveal mobile:w-[min(420px,_100%)] mobile:m-auto [&.is-waiting]:opacity-0
        [&.is-waiting]:[transform:translateY(35px)] [transition:opacity_0.75s_ease,_transform_0.75s_ease]
        reduce-motion:[&.is-waiting]:opacity-[1] reduce-motion:[&.is-waiting]:[transform:none]"><x-project-previews /></div>
                                @endif
                                @if ($service['id'] === 'mobile-applications')
                                    <div class="detail-art reveal mobile:w-[min(420px,_100%)] mobile:m-auto [&.is-waiting]:opacity-0
        [&.is-waiting]:[transform:translateY(35px)] [transition:opacity_0.75s_ease,_transform_0.75s_ease]
        reduce-motion:[&.is-waiting]:opacity-[1] reduce-motion:[&.is-waiting]:[transform:none]"><x-project-previews :mobile="true" /></div>
                                @endif
                            </div>
                        </div>
                    </section>
        ...
        gap-[90px] items-start tablet:gap-10 mobile:grid-cols-1 mobile:gap-10">
                    <div>
                        <p class="eyebrow text-[11px] font-semibold tracking-[2.7px] text-[#a2a8ff] mb-[22px] mobile:mb-[18px] desktop:text-[13px]">LET’S MAKE IT HAPPEN</p>
                        <p class="footer-brand text-[clamp(34px,_5.3vw,_72px)] tracking-[0.13em] leading-[1] mb-[25px]
        mobile:text-[clamp(35px,_9vw,_60px)]">VALOURITE</p>
                        <h2 class="leading-[1.15] text-[clamp(29px,_3vw,_46px)] font-semibold tracking-[-1.2px] desktop:text-[clamp(38px,_3.4vw,_54px)]">Let’s build something useful.</h2>
                        <p class="contact-intro text-[var(--muted)] text-[14px] mt-5 desktop:text-[18px]">Tell us what you have in mind.<br>We work with businesses across South Africa.</p>
                        <div class="contact-people grid grid-cols-2 gap-[25px] mt-8 mobile:gap-5">
        ...
        desktop:[&_h3]:text-[22px] desktop:[&_.text-link]:text-[15px]">
                                    <h3 class="leading-[1.15] font-semibold">{{ $contact['name'] }}</h3><a class="[-webkit-tap-highlight-color:transparent] touch-manipulation no-underline
        focus-visible:[outline:2px_solid_#7dd3fc] focus-visible:outline-offset-[6px]" href="tel:+{{ $contact['number'] }}">{{ $contact['display'] }}</a>
                                    <div class="flex gap-5 "><a class="[-webkit-tap-highlight-color:transparent] touch-manipulation no-underline
        focus-visible:[outline:2px_solid_#7dd3fc] focus-visible:outline-offset-[6px] text-link inline-flex
        items-center gap-[9px] text-[14px] [&_.icon]:text-[#999bff] [&_.icon]:w-5 hover:text-[#9edaff]
        ...
        focus-visible:[outline:2px_solid_#7dd3fc] focus-visible:outline-offset-[6px] text-link inline-flex
        items-center gap-[9px] text-[14px] [&_.icon]:text-[#999bff] [&_.icon]:w-5 hover:text-[#9edaff]
        desktop:text-[17px]" href="tel:+{{ $contact['number'] }}"><x-icon name="phone" />Call</a></div>
                                </div>
                            @endforeach
        ...
        hover:text-[#9edaff] desktop:text-[17px] mt-7" href="mailto:{{ config('website.email') }}"><x-icon name="mail" />{{ config('website.email') }}</a>
                    </div>
        ...
        desktop:[&_input]:text-[17px] desktop:[&_textarea]:text-[17px]" method="post" action="{{ route('enquiries.store') }}#contact">
                        ...
                        @if (session('enquiry_sent'))
                            <p class="form-success p-[14px] [border:1px_solid_#a6777c] [background:#0e302a] rounded-[6px] text-[13px] border-[#448d7a]" role="status">{{ session('enquiry_sent') }}</p>
                        @endif
                        @if ($errors->any())
                            <div class="form-errors p-[14px] [border:1px_solid_#a6777c] [background:#331c2b] rounded-[6px] text-[13px] [&_ul]:pl-4
        [&_ul]:list-disc" role="alert">
                        ...
                        <textarea class="[-webkit-tap-highlight-color:transparent] focus:[outline:2px_solid_#8494ff] focus:outline-offset-[2px]" id="message" name="message" rows="4" placeholder="Tell us a little about your project…" required minlength="10" maxlength="5000" @error('message') aria-invalid="true" @enderror>{{ old('message') }}</textarea>
        ...
        desktop:text-[13px]">
                    <p>© {{ date('Y') }} Valourite. All rights reserved.</p><a class="[-webkit-tap-highlight-color:transparent] touch-manipulation no-underline
        focus-visible:[outline:2px_solid_#7dd3fc] focus-visible:outline-offset-[6px]" href="#main">Back to top ↑</a><span>IDEAS → SYSTEMS → PROGRESS</span>
                </div>
            </div>
        </footer>
    <script>
